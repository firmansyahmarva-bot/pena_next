...
import { Shield, Clock, Award, CheckCircle2, ArrowRight, BookOpen, AlertTriangle, Users, Building, HelpCircle, FileText, Send, Calendar, CheckSquare } from 'lucide-react';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: program.meta_title || `${program.title} 2026 — PENA Consultant`,
...
export default async function ProgramPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const programFaqs = [
    {
      question: `Berapa biaya pelatihan ${program.name} di PENA Consultant?`,
      answer: program.base_price 
        ? `Biaya investasi pelatihan ${program.name} mulai dari Rp ${program.base_price.toLocaleString('id-ID')} per peserta. Biaya ini sudah termasuk bimbingan pembinaan, ujian sertifikasi resmi, modul materi lengkap, training kit eksklusif, dan penerbitan sertifikat/SKP resmi.`
        : `Biaya pelatihan ${program.name} bervariasi sesuai skema public batch atau corporate in-house training. Silakan hubungi tim konsultan kami untuk penawaran harga terbaik.`,
    },
    {
      question: `Berapa lama durasi pelaksanaan pembinaan ${program.name}?`,
      answer: `Durasi pelatihan ${program.name} adalah ${program.duration || '12 hari kerja'}, diselenggarakan secara intensif dengan kombinasi teori regulasi, studi kasus analisis risiko, praktik kerja lapangan (PKL), dan evaluasi ujian akhir.`,
    },
    {
      question: `Apakah sertifikat ${program.name} resmi dan terdaftar di kementerian?`,
      answer: isKemnaker 
    ...
    {
      question: `Apakah peserta fresh graduate / lulusan baru boleh mendaftar?`,
      answer: `Ya, lulusan baru (Fresh Graduate) minimal D3/D4/S1 semua jurusan diperbolehkan mengikuti pembinaan untuk memperoleh Sertifikat Pembinaan Calon Ahli K3 sebagai nilai tambah karir di bidang HSE.`,
    },
  ];

  const breadcrumbs = [
    { name: 'Home', url: 'https://penaconsultant.co.id' },
    { name: 'Pelatihan K3', url: 'https://penaconsultant.co.id/pelatihan' },
    { name: program.name, url: `https://penaconsultant.co.id/pelatihan/${program.slug}` },
  ];
  ...
  return (
    <>
      ...
      <div className="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="p-6 bg-gradient-to-r from-emerald-50 to-primary-50 border border-emerald-200 rounded-2xl">
          <div className="flex items-start gap-3.5">
            <CheckSquare className="w-6 h-6 text-emerald-700 shrink-0 mt-0.5" />
            <div className="space-y-1 text-sm text-slate-800">
              <strong className="block text-slate-950 text-base">
                Ringkasan Resmi Pelatihan {program.name} 2026:
              </strong>
              <p className="leading-relaxed">
                Pelatihan <strong>{program.name}</strong> di PENA Consultant adalah program pembinaan kompetensi K3 resmi bersertifikat <strong>{isKemnaker ? 'Kemnaker RI' : 'BNSP'}</strong> dengan durasi <strong>{program.duration || '12 hari kerja'}</strong> dan biaya investasi mulai dari <strong>{program.base_price ? `Rp ${program.base_price.toLocaleString('id-ID')}` : 'Rp 7.500.000'}</strong>. Lulusan berhak memperoleh sertifikat resmi, lisensi K3, dan modul lengkap.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Program FAQ Accordion (Schema-Enriched) */}
            <section className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
                <HelpCircle className="w-5 h-5 text-primary-600" /> FAQ — Pertanyaan Seputar Pelatihan {program.name}
              </h2>
              <div className="space-y-3">
                {programFaqs.map((faq, i) => (
                  <details key={i} className="p-4 bg-slate-50 rounded-xl border border-slate-200 group">
            ...
            {relatedArticles.length > 0 && (
              <section className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
                <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-primary-600" /> Panduan Terkait {program.name}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {relatedArticles.map((art) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-bold text-primary-700">Baca Panduan Lengkap →</span>
                    </Link>
                  ))}
                </div>
              </section>
